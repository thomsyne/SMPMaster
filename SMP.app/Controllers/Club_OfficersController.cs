...
{
    public class Club_OfficersController : Controller
    {
        // GET: ClubOfficers
        IEnumerable<Club_Officers> Club_OfficersList = null;
        public ActionResult Index(int id = 0, int taskid = 7)
        {
            try
            {
                ...
                HttpResponseMessage resp = GlobalVariables.client.GetAsync("GetClub_Officers?id=" + id + "&taskid=" + taskid).Result;
                if (resp.IsSuccessStatusCode)
                {
                    Club_OfficersList = resp.Content.ReadAsAsync<IEnumerable<Club_Officers>>().Result;
                }
                else
                {
                    ModelState.AddModelError(string.Empty, "Server error. Please contact administrator.");
                }
                return View(Club_OfficersList);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult AddOrEdit(int id = 0, int taskid = 0)
        {
            ...
            else
            {
                ...
                if (resp.IsSuccessStatusCode)
                {
                    var clu = resp.Content.ReadAsAsync<IEnumerable<Club_Officers>>().Result;
                    var record = clu.FirstOrDefault();
                    return View(record);
                }
                return View();
            }
        ...
        [HttpPost]
        public ActionResult AddOrEdit(Club_Officers model)
        {
            BindCombo();
            if (model.ItbId == 0)
            {
                model.taskid = 2;
                HttpResponseMessage resp = GlobalVariables.client.PostAsJsonAsync("POSTClub_Officers", model).Result;
        ...
        void BindCombo()
        {
            ...
            HttpResponseMessage resp12 = GlobalVariables.client.GetAsync("GetSchool_Details?id=" + id + "&taskid=" + taskid).Result;
            School_DetailsList = resp12.Content.ReadAsAsync<IEnumerable<School_Details>>().Result;
            ...
            HttpResponseMessage resp5 = GlobalVariables.client.GetAsync("GetClubs?id=" + id + "&taskid=" + taskid).Result;
            ClubsList = resp5.Content.ReadAsAsync<IEnumerable<Clubs>>().Result;
            ...
            IEnumerable<Club_Membership> Club_MembershipList = null;
            HttpResponseMessage resp6 = GlobalVariables.client.GetAsync("GetClub_Membership?id=" + id + "&taskid=" + taskid).Result;
            Club_MembershipList = resp6.Content.ReadAsAsync<IEnumerable<Club_Membership>>().Result;
            if (!Equals(Club_MembershipList, null))
            {
                var clu = Club_MembershipList.ToList();
                ViewBag.Club_MembershipList = new SelectList(clu, "ItbId", "StudentId");
            }
