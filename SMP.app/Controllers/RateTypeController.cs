    ...
    public class RateTypeController : Controller
    {
        ...
        public ActionResult AddOrEdit(int id = 0, int taskid = 0)
        {
            ...
            else
            {
                ...
                if (resp.IsSuccessStatusCode)
                {
                    var RateType = resp.Content.ReadAsAsync<IEnumerable<RateType>>().Result;
                    var record = RateType.FirstOrDefault();
                    return View(record);
                }
                return View();
            }
        ...
        [HttpPost]
        public ActionResult AddOrEdit(RateType model)
        {
            if (model.ItbId == 0)
            {
                model.taskid = 2;
                HttpResponseMessage resp = GlobalVariables.client.PostAsJsonAsync("POSTRateType", model).Result;
