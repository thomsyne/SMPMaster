    ...
    public class CountryController : Controller
    {
        ...
        public ActionResult AddOrEdit(int id = 0, int taskid = 0)
        {
            ...
            else
            {
                ...
                if (resp.IsSuccessStatusCode)
                {
                    var countries = resp.Content.ReadAsAsync<IEnumerable<Country>>().Result;
                    var record = countries.FirstOrDefault();
                    return View(record);
                }
                return View();
            }
        ...
        [HttpPost]
        public ActionResult AddOrEdit(Country model)
        {

            if (model.ItbId == 0)
            {
                model.taskid = 2;
                HttpResponseMessage resp = GlobalVariables.client.PostAsJsonAsync("POSTCountry", model).Result;
