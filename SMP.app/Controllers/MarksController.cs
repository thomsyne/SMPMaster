...
namespace SMP.Controllers
{
    public class MarksController : Controller
    {
        void BindCombo()
        {
            ...
            HttpResponseMessage resp5 = GlobalVariables.client.GetAsync("GetProgram?id=" + id + "&taskid=" + taskid).Result;
            ProgramList = resp5.Content.ReadAsAsync<IEnumerable<Program>>().Result;
            ...
            HttpResponseMessage resp6 = GlobalVariables.client.GetAsync("GetExam_Type?id=" + id + "&taskid=" + taskid).Result;
            Exam_TypeList = resp6.Content.ReadAsAsync<IEnumerable<Exam_Type>>().Result;
            if (!Equals(Exam_TypeList, null))
            {
                var Exam_Types = Exam_TypeList.ToList();
                ViewBag.Exam_TypeList = new SelectList(Exam_Types, "ItbId", "Description");
            }

            IEnumerable<School_Details> School_DetailsList = null;
            HttpResponseMessage resp7 = GlobalVariables.client.GetAsync("GetSchool_Details?id=" + id + "&taskid=" + taskid).Result;
            School_DetailsList = resp7.Content.ReadAsAsync<IEnumerable<School_Details>>().Result;
            ...
            HttpResponseMessage resp8 = GlobalVariables.client.GetAsync("GetStudent?id=" + id + "&taskid=" + taskid).Result;
            StudentList = resp8.Content.ReadAsAsync<IEnumerable<Student>>().Result;
        ...
        IEnumerable<Mark> MarkList = null;
        // GET: Mark
        public ActionResult Index(int id = 0, int taskid = 7)
        {
            try
            {
                HttpResponseMessage resp = GlobalVariables.client.GetAsync("GetMarks?id=" + id + "&taskid=" + taskid).Result;
                if (resp.IsSuccessStatusCode)
                {
                    MarkList = resp.Content.ReadAsAsync<IEnumerable<Mark>>().Result;
                }
                else
                {
        ...
        public ActionResult AddOrEdit(int id = 0, int taskid = 0)
        {
            ...
            else
            {
                ...
                if (resp.IsSuccessStatusCode)
                {
                    var Mark = resp.Content.ReadAsAsync<IEnumerable<Mark>>().Result;
                    var record = Mark.FirstOrDefault();
                    return View(record);
                }
                return View();
            }
        ...
        [HttpPost]
        public ActionResult AddOrEdit(Mark model)
        {
            BindCombo();
            if (model.ItbId == 0)
            {
                model.taskid = 2;
                HttpResponseMessage resp = GlobalVariables.client.PostAsJsonAsync("POSTMarks", model).Result;
